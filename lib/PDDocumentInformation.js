...
const DateToJavaCalendar = (date) => {
    let dateLong = java.newInstanceSync("java.lang.Long", date.getTime().toString());
    let javaDate = new javaImports.Date(dateLong);
...
class PDDocumentInformation extends JavaClass {
     ...
     */
    getCreationDate() {
        return new Date(this.javaClass.getCreationDateSync().getTimeSync().getTimeSync());
    }
     ...
     */
    getModificationDate() {
        return new Date(this.javaClass.getModificationDateSync().getTimeSync().getTimeSync());
    }
