...
class PDDocument extends JavaClass {
     ...
     */
    static loadFromPathAsync(path, password) {
        return new Promise(async (resolve, reject) => {
            const file = new javaImports.File(path);
            if (typeof password !== "undefined")
                resolve(new PDDocument(await javaImports.PDDocument.load(file, password)));
            resolve(new PDDocument(await javaImports.PDDocument.load(file)));
        });
    }
